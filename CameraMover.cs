...
public class CameraMover : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (shouldMove)
        {
            ...
            targetPosition += Vector3.right * currentSpeed * Time.deltaTime;
        }
    }
    ...
    void LateUpdate()
    {
        if (shouldMove)
        {
            float dynamicLerp = Mathf.Clamp(currentSpeed / maxSpeed, 0.1f, 1f);
            transform.position = Vector3.Lerp(transform.position, targetPosition, dynamicLerp);
        }
    }
}
